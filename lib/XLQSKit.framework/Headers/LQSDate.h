...
/**
 ...
 2020-08-22 17:17:00 -> 1598087820
 */
+ (NSString*) secondsStringWithDateString:(NSString *)dateString;
+ (NSString*) secondsStringWithDateString:(NSString *)dateString format:(NSString *)format;
+ (NSString*) secondsStringWithDateString:(NSString *)dateString format:(NSString *)format timeZone:(NSString *)timeZoneName;
...
#pragma mark - 处理时间戳（推荐 2019-08-21）
/**
 秒（10位）、毫秒（13位） NSString、NSNumber 的时间戳转成指定时间格式 DateFormat 的字符串。
 默认 DateFormat 为 yyyy-MM-dd HH:mm:ss
 */
+ (NSString *) dateStringFromTimestamp:(id)timestamp;
+ (NSString *) dateStringFromTimestamp:(id)timestamp withDateFormat:(NSString*)dateFormat;
+ (NSString *) dateStringFromTimestamp:(id)timestamp withDateFormat:(NSString*)dateFormat withTimeZone:(NSString*)timeZone;


+ (NSDate *) dateFromTimestamp:(id)timestamp;


+ (NSString *) timestampString10Bit;
+ (NSString *) timestampString13Bit;

+ (NSString *) stringWithDateFormat:(NSString *)dateFormat contentString:(NSString *)contentString contentFormat:(NSString *)contentFormat;


/**
 输出：星期日、星期一、星期二、星期三、星期四、星期五、星期六
 */
+ (NSString *) weekCNStringWithContentString:(NSString *)contentString contentFormat:(NSString *)contentFormat;

/**
 contentFormat 为 yyyy-MM-dd
 */
+ (NSString *) weekCNStringWithContentString:(NSString *)contentString;
